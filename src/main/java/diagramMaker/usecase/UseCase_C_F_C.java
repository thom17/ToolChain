...
public class UseCase_C_F_C extends Base
{
	static int idNum=0;
	...
	public static void main(DataList datalist) throws IOException
	{
		...
		String src = "result/usecase/"+time+"/C_F_C";
		File folder = new File(src);
		folder.mkdir();
	...
	private static void getUseCase(Function system, Class actor, StringBuilder sb) 
	{
		for(Class client : system.getCallList().getClassList())
		{
			if(actor != client)
			{
