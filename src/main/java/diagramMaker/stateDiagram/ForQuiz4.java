...
{
	static final boolean codePrint = false;
	...
	{
		Process p = Runtime.getRuntime().exec("java -jar lib/plantuml.jar result/quiz4.txt");
		p.waitFor();
	...
	public static void main(DataList datalist) throws IOException, InterruptedException
	{
		...
		Process p = Runtime.getRuntime().exec("java -jar lib/plantuml.jar -tsvg result/quiz4.txt");
		p.waitFor();
	...
	private static void makeMemberInfo(Class target, StringBuilder sb)
	{
		...
		for(Member var : memberList)
		{
			...
			String imageList = var.getImageList().toString();
			sb.append("\t\tstate " + str + " as " + key+":"+imageList+"\n");			
		}
		sb.append("\t}\n");
	}
